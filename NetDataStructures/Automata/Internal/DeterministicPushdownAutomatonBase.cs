...
{
    public abstract class DeterministicPushdownAutomatonBase
    {
        /// <summary>
        ...
        /// </summary>
        public IReadOnlyDictionary<(string State, char? Symbol, char Pop), (string State, string Push)>
            Delta => _delta;
        ...
        private Dictionary<(string State, char? Symbol, char Pop), (string State, string Push)> _delta;

        protected DeterministicPushdownAutomatonBase(
        ...
        private void Initialize(ImmutableHashSet<char> alphabet, ImmutableHashSet<char> stackAlphabet,
            ...
            Dictionary<(string State, char? Symbol, char Pop), (string State, string Push)> delta)
        {
        ...
        private bool RunInternal(List<char> stack, string state, string input, int index)
        {
            ...
            if (index < input.Length)
            {
                ...
                {
                    // Run recursively. Note the index is incremented bc we consumed a symbol
                    if (RunInternal(stack, input, index + 1, edge))
                    {
                        return true;
                    }
                }
            ...
            else if (_delta.TryGetValue((state, null, stack[^1]), out var ncEdge))
            {
                // Run recursively 
                if (RunInternal(stack, input, index, ncEdge))
                {
                    return true;
                }
            }
